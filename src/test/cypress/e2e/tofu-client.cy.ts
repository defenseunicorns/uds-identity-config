...
describe("Tofu Client Management", () => {
    ...
    it("should fail with invalid OpenTofu configuration", () => {
        if (!credentials) throw new Error('Credentials not initialized');
        ...
        const invalidConfig = `
            ...
            resource "keycloak_openid_client" "invalid_client" {
                realm_id  = "nonexistent-realm"
                client_id = "invalid-client"
                name      = "invalid-client"

                # Missing required fields like access_type, etc.
            }
        `;

        // Write the invalid config to a temporary file
        const tempDir = "./test-tf-invalid";
        cy.exec(`mkdir -p ${tempDir}`);
        cy.writeFile(`${tempDir}/main.tf`, invalidConfig);

        // Initialize OpenTofu
        return cy.exec(`cd ${tempDir} && tofu init`, { failOnNonZeroExit: false })
            .then(() => {
                // Try to apply with invalid configuration
                return cy.exec(
                    `cd ${tempDir} && tofu apply -auto-approve -var="keycloak_client_secret=${clientSecret}"`,
                    { failOnNonZeroExit: false }
                );
            })
            .then((result) => {
