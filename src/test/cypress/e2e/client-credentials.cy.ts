...
describe("UDS Operator Client Credentials", () => {
      ...
      cy.request({
        ...
        cy.request({
          ...
          expect(response.body).to.deep.equal({
            ...
            error_description:
              "The Client doesn't have the created-by=uds-operator attribute. Rejecting request.",
          });
        });
      });
    });
  ...
  it("UDSClientPolicyPermissionsExecutor validates mappers and claims", () => {
    cy.task(
      "exec",
      "uds zarf tools kubectl apply -f ./resources/test-package-not-passing-validation.yaml",
    )
      .its("exitCode")
      .should("eq", 0);
    cy.task(
      "exec",
      "uds zarf tools kubectl wait --for=condition=Ready=false package/test-package-not-passing-validation -n test-package-not-passing-validation --timeout=300s",
    )
      .its("exitCode")
      .should("eq", 0);
    cy.task("exec", "kubectl get events -n test-package-not-passing-validation")
      ...
      .should(
        "include",
        '{"error":"invalid_client","error_description":"The Protocol Mapper non-whitelisted-protocol-mapper is not allowed. Rejecting request."}',
      );
    ...
    cy.getAccessToken().then((accessToken: string) => {
      cy.request({
        ...
        expect(response.status).to.eq(200);
        const testClient = response.body.find(
          (client: any) => client.clientId === "test-package-not-passing-validation",
        );
  ...
  it("UDSClientPolicyPermissionsExecutor supports adding additional Protocol Mappers declaratively", () => {
    cy.task(
      "exec",
      "uds zarf tools kubectl apply -f ./resources/test-package-allowed-protocol-mapper.yaml",
    )
      .its("exitCode")
      .should("eq", 0);
    cy.task(
      "exec",
      "uds zarf tools kubectl wait --for=condition=Ready=true package/test-package-allowed-protocol-mapper -n test-package-allowed-protocol-mapper --timeout=300s",
    )
      .its("exitCode")
      .should("eq", 0);
  });
