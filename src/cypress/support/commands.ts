...
Cypress.Commands.add('registrationPage', (formData: RegistrationFormData) => {
    ...
    cy.get('label').contains('Unit, Organization or Company Name').next('input').type(formData.organization);
    ...
    cy.get('body').should('be.visible').type('Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec vitae nunc nec est mattis faucibus ac at justo. Nullam auctor tellus nec sapien tristique, eget feugiat dolor accumsan. Duis sit amet aliquet sapien. Suspendisse non felis et ante posuere dapibus.');
