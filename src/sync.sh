...
mkdir -p /opt/keycloak/truststore/
...
if [ -z "${CUSTOM_TRUSTSTORE}" ]; then
  cp -fvi "$CUSTOM_TRUSTSTORE" /opt/keycloak/truststore/truststore.jks
else
  cp -fvu truststore.jks /opt/keycloak/truststore/truststore.jks
fi
