...
2. **Create a ConfigMap with your image assets**
   ...
   Generate a ConfigMap manifest using `kubectl`. Adjust the file paths and include only the images you want to override:
...
```bash
kubectl apply -f theme-image-cm.yaml -n keycloak
...
```bash
kubectl get configmap keycloak-theme-overrides -n keycloak -o yaml
```
...
```bash
kubectl get configmap keycloak-theme-overrides -n keycloak \
