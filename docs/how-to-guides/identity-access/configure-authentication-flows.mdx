...
```bash
# View realm authentication flow settings in Keycloak admin UI
# Navigate to: keycloak.<admin_domain> → uds realm → Authentication → Flows
```
...
- path: realmInitEnv
  value:
    X509_OCSP_FAIL_OPEN: "true"     # Allow auth if OCSP responder is unreachable
    # OR
    X509_OCSP_CHECKING_ENABLED: "false"  # Disable OCSP entirely
```
